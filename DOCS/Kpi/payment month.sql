...
base AS (
  SELECT o.order_id, STRFTIME('%Y-%m', o.order_purchase_timestamp) AS ym
...
items AS (
  SELECT oi.order_id,
         ...
         CASE WHEN oi.freight_value IS NULL OR oi.freight_value < 0 THEN 0 ELSE oi.freight_value END AS freight_value
  FROM order_items oi
...
pay AS (
  SELECT order_id, SUM(CASE WHEN payment_value >= 0 THEN payment_value ELSE 0 END) AS paid
